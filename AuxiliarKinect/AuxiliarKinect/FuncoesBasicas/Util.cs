...
namespace AuxiliarKinect.FuncoesBasicas
{
    ...
    {
        public static bool CompararComMargemErro(double pMargemErro, double pValor1, double pValor2)
        {
            return pValor1 >= pValor2 - pMargemErro
                && pValor1 <= pValor2 + pMargemErro;
        }
        ...
        private static Vector4 CriarVetorEntreDoisPontos(Joint articulacao1, Joint articulacao2)
        {
            ...
            vetorResultante.X = Convert.ToSingle(
                Math.Sqrt(Math.Pow(articulacao1.Position.X - articulacao2.Position.X, 2)));

            vetorResultante.Y = Convert.ToSingle(
                Math.Sqrt(Math.Pow(articulacao1.Position.Y - articulacao2.Position.Y, 2)));

            vetorResultante.Z = Convert.ToSingle(
                Math.Sqrt(Math.Pow(articulacao1.Position.Z - articulacao2.Position.Z, 2)));
        ...
        public static double CalcularProdutoEscalar(Joint articulacao1, Joint articulacao2,
            Joint articulacao3)
        {
            Vector4 vetorV =
